...
const TvHeroSection = () => {
  ...
  useEffect(() => {
    if (!Todaylist[value]?.backdrop_path) return;

    setIsLoaded(false);
    setImageError(false);

    const newImageUrl = `https://image.tmdb.org/t/p/w1280/${Todaylist[value].backdrop_path}`;
  ...
  const currentMovie = Todaylist[value];
